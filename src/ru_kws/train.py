...
def main():
    ...
    (args.run_dir / "config.yaml").write_text(yaml.safe_dump(config, sort_keys=False), encoding="utf-8")
    write_json(args.run_dir / "labels.json", labels)
    try:
        ...
        revision = None
    write_json(args.run_dir / "environment.json", {
    ...
    with (args.run_dir / "history.csv").open("w", newline="", encoding="utf-8") as history:
        ...
        for epoch in range(1, settings["max_epochs"] + 1):
            ...
            save_checkpoint(args.run_dir / "last.pt", payload)
            if improved:
                save_checkpoint(args.run_dir / "best.pt", payload)
            ...
            if stale >= settings["early_stopping_patience"]:
                ...
                break
    print(f"Best checkpoint: {args.run_dir / 'best.pt'}", flush=True)
